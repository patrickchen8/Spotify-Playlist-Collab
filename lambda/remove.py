...
def lambda_handler(event, context):
    try: 
        ...
        if uri is None: 
            return {
                'statusCode': 400,
                'body': "Song doesn't exist"
            }

        sql = "SELCT * FROM playlist WHERE songID = %s AND userid = %s"
        params = [uri, userid]
        ...
        if row == ():
            return {
                'statusCode': 400,
                'body': "Song not in playlist or song was not added by you"
            }
        ...
        if response.status_code != 200:
            return {
                'statusCode': 400,
                'body': "Error deleting the song from playlust"
            }
        
        sql = "DELETE FROM playlist WHERE songID = %s"
        params = [uri]
        try:
            datatier.perform_action(dbConn, sql, params)
            return {
                'statusCode': 200,
                'body': f'Deleted {song} from the playlist'
            }
